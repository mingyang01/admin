...
<html lang="en">
	<head>
	    ...
	    <script type="text/javascript" src="http://webapi.amap.com/maps?v=1.3&key=3ff6bb8845e8210c8819b70b663192bc"></script>
	    <script type="text/javascript" src="http://webapi.amap.com/maps?v=1.3&key=3ff6bb8845e8210c8819b70b663192bc&plugin=AMap.Geocoder"></script>
...
<script type="text/javascript">
    ...
    var infoWindow = new AMap.InfoWindow({offset: new AMap.Pixel(0, -30)});
   	function makers(markers){
   		markers.forEach(function(marker) {
   			var position = marker['location'].split(';');
	        var _marker = new AMap.Marker({
	            map: map,
	            position: [position[1], position[0]]
	        });
	        _marker.content = "<div>"+marker['title']+"</div>"
	        				+ "<div>地址:"+marker['address']+"</div>"
	        				+ "<div>时间:"+marker['time']+"</div>"
	        				+ "<div>主办:"+marker['host']+"</div>"
	        				+ "<div>费用:"+marker['pay']+"</div>"
	        				+"<img src="+"'"+marker['imgurl']+"'"+" width=200 height=200/>"
    ...
    function regeocoder(lnglatXY) {  //逆地理编码
        ...
        geocoder.getAddress(lnglatXY, function(status, result) {
            if (status === 'complete' && result.info === 'OK') {
            	var citycode = result['regeocode']['addressComponent']['citycode']?result['regeocode']['addressComponent']['citycode']:'';
            	if(citycode){
            		var url = '/Home/event/getAllEvent';
            		var params = {
            			citycode : citycode
            		}
